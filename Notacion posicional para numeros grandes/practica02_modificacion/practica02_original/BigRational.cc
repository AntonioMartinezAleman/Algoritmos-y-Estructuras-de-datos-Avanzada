...
template <unsigned char Base>
BigRational<Base> BigRational<Base>::media(const std::vector<BigRational<Base>&> vector_rationals) const {
  BigRational<Base> suma = BigUnsigned<Base>(0u)
  for(int i = 0; i < vector_rationals.size(); i++) {
    suma += vector_rationals[i];
  }
  suma /= vector_rationals.size();
  return suma;
}
